...
# ВАЖНО: FreePBX Web interface - ОГРАНИЧЕН по IP (защита от CVE-2024-45602)
# Замените 162.120.187.223 на ваш реальный IP!
TRUSTED_ADMIN_IP="162.120.187.223"
log "Ограничиваем доступ к FreePBX Web только для $TRUSTED_ADMIN_IP"
ufw allow from $TRUSTED_ADMIN_IP to any port 8080 proto tcp comment "FreePBX Web - Trusted IP only"
ufw allow from $TRUSTED_ADMIN_IP to any port 8443 proto tcp comment "FreePBX HTTPS - Trusted IP only"
...
echo "4. Обновите FreePBX до последней версии"
echo "5. Перезагрузите сервер для применения всех изменений"
echo ""
echo "🔒 ЗАЩИТА ОТ CVE-2024-45602:"
echo "✅ Веб-интерфейс доступен только с $TRUSTED_ADMIN_IP"
echo "✅ Если ваш IP изменится - обновите настройки UFW"
echo "✅ SIP порты открыты для работы телефонии"
echo ""
echo "🚨 ВАЖНО:"  
echo "• Убедитесь что у вас есть доступ по SSH перед перезагрузкой"
echo "• Проверьте доступ к веб-интерфейсу: http://37.27.240.184:8080"
echo "• При смене IP обновите правила: ufw delete allow 8080 && ufw allow from НОВЫЙ_IP to any port 8080"